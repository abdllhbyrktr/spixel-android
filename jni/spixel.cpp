...
Java_net_bayraktaryazilim_myopencvnative_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
    return env->NewStringUTF("Don't forget to add 'config.yml' file for processing entered image file.");
}

extern "C" JNIEXPORT jstring JNICALL
Java_net_bayraktaryazilim_myopencvnative_MainActivity_processImage(
        JNIEnv *env,
        jobject /* this */,
        jstring config,
        jstring image) {

    string configfile = env->GetStringUTFChars(config, NULL);
    string imagefile = env->GetStringUTFChars(image, NULL);
    string filename = FileName(imagefile);
    string filedir = FilePath(imagefile);
    string message = filename + "'s processing done. Check out this folder to see the results: " + filedir;
    ProcessFiles(configfile, imagefile, "", "");
